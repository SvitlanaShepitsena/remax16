                    ...
                    return $q(function (resolve, reject) {
                        if (isBroker) {
                            ...
                            refSaleRent.$loaded().then(function (filteredHomes) {
                                ...
                                if (filteredHomes.length === 0 && queryObj.address) {
                                    NearestListingsServ.find(queryObj.address, refSaleRent).then(function (nearestHomes) {

                                        GetAgentsInfoServ.get(nearestHomes).then(function (homesWithAgents) {
                                            resolve(homesWithAgents);
                                        })
